<template>
  <div id="export-and-import-data-page">
    ...
    <t-button @click="exportTtsHistory" class="btn" size="large" theme="primary" block>导出 TTS 记录</t-button>
    <t-button @click="ttsFileInput.click()" class="btn" size="large" theme="primary" block>导入 TTS 记录</t-button>
    <!--导入OCR记录的文件表单-->
    ...
    <!--导入TTS记录的文件表单-->
    <input type="file" id="tts-input" v-show="false" ref="ttsFileInput" @change="importTtsHistory">
  </div>
</template>
 ...
 * @returns {Promise<boolean>}
 */
async function importTtsHistory() {
  if (ttsFileInput.value.value === '') return false;
  // 不是 JSON
  if (!/\.json$/.test(ttsFileInput.value.files[0].name)) {
    await Dialog.alert({
      title: '不支持的配置文件',
      message: `您选择的 ${ttsFileInput.value.files[0].name} 不是支持的配置文件，请选择 JSON 文件！`,
      buttonTitle: '知道了'
    });
    return false;
  }
  // 读取文件
  const reader = new FileReader();
  reader.readAsText(ttsFileInput.value.files[0]);
  // 读取完成
  reader.addEventListener('load', async ev => {
    try {
      const jsonData = JSON.parse(ev.target.result);
      const result = await data.importTtsHistory(jsonData);
      if (result.result !== 'success') {
        await Dialog.alert({
          title: '导入数据出错',
          message: result.msg,
 ...
 * @returns {Promise<boolean>}
 */
async function exportTtsHistory() {
  // 获取 TTS 记录
  const result = await data.exportTtsHistory();
  if (result.result !== 'success') {
    await Dialog.alert({
      title: '出错了',
      message: result.msg,
  ...
  try {
    ...
    // 将字符串写入应用的 Cache 目录
    const writeResult = await Filesystem.writeFile({
      path: 'OCRanslate-tts-history.json',
      data: jsonStr,
      directory: Directory.Cache,
      encoding: 'utf8'
    });
    ...
    await Toast.show({text: '已成功导出 TTS 记录'});
  }catch (error) {
    if (error.message !== 'Share canceled') {
      await Dialog.alert({
        title: '出错了',
