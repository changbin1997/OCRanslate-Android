<template>
  <div id="translation-history-page">
...
<script setup>
document.title = '翻译历史记录 - OCRanslate';
...
async function getOcrHistory() {
  ...
  // 获取总数量
  const countResult = await data.getTranslationHistoryCount();
  ...
  // 继续获取记录
  const result = await data.getTranslationHistory(start, 20);
...
/*标题和数量区域*/
#translation-history-page .title-and-count {
  padding: 16px 0;
  color: #666666;
}
